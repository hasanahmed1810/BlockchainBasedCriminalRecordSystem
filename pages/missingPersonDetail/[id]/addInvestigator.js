...
function Investigation({ id }) {
  ...
  async function submit(event) {
    ...
    const transaction = await missingPerson(id)
      .methods.setInvestigator(name, civID, badge, rank)
      .send({
        from: "0x19EB8fcE962B24acf466dbA05B52Aa299B24Ac27",
        gas: 6721975,
      });
    await setDoc(
      doc(db, "add Investigator (missing person) transactions", transaction.transactionHash),
      transaction
    );
    router.push("/missingPersonDetail/" + id);
  }
